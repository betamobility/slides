...
- **KTD13. The design system's own favicon and wordmark, vendored, never redrawn.** (Corrected during execution, Johan 2026-09-08: no new mark.) `beta/logo/` vendors `favicon-16.png`, `favicon-32.png`, `apple-touch-icon.png` and the recolorable `wordmark.svg` from `Tools/design-system/public/logo/` with provenance, the way `beta/fonts/` vendors the faces. `slides/index.html` carries `favicon-32.png` as a base64 data URI and the boot splash inlines `wordmark.svg` charcoal on cream; `slides/src/beta/marks.ts` is the runtime copy for the About header. `scripts/test-beta-marks.ts` pins the vendored files to the design-system SHA-256 hashes and asserts the copies are byte-identical. The release site's published pages use the design system's three link tags and `release.mjs` copies `beta/logo/*.png` into `site/logo/`.
- **KTD14. The product is `beta/slides`.** (Added during execution, Johan 2026-09-08.) `appName` in `slides/src/main.ts` is `beta/slides`, which feeds the window title, the file-picker label and the About dialog; the static page title and the splash word follow, and the About tooltip key moves in all eight catalogues. The document `format` stays `bento/slides`: that is the file format, not the product.
