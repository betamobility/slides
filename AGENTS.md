...
This repository is Beta Mobility's fork of `nyblnet/bento` that builds
`bento/slides` only. Everything above is upstream's contract and stays
verbatim; this section is what the fork adds. The plan of record is
`docs/plans/2026-09-08-001-feat-beta-slides-bento-fork-plan.md`; where it and
`docs/PLATFORM.md` disagree, PLATFORM wins.
...
1. **Kernel-zone edits are three files, named, and offered upstream.**
   ...
   first. Branches for upstream carry no attribution trailers (upstream rule 8).
2. **Beta additions live where upstream cannot collide with them.** `beta/`
   for tokens, fonts, theme and templates; `plugins/beta-slides/` for the
   skill; `scripts/build-beta-*` and `scripts/test-beta-*` for tooling. Inside
   `slides/src/`, an addition is one `case` block or one new file
   (`slides/src/export/pptx.ts`), never a rewrite.
3. **Design system goes through Bento's keys.** `doc.theme`, `doc.fonts`,
   `doc.layouts` with `role`, `doc.meta` tokens. No parallel Beta theme layer.
...
6. **Commits in this fork carry Beta's session attribution** (the
   `Co-Authored-By` and `Claude-Session` trailers Beta's harness requires).
...
`@AGENTS.md`. Upstream keeps a rich `CLAUDE.md`; its content is folded in
below, verbatim, so nothing upstream wrote is lost. On each upstream merge,
resolve `CLAUDE.md` back to the wrapper and re-sync the section below.
